...
public class Course {
    ...
    // Score Functions
    public float calculateOverallGrade(int StudentID){
    ...
    public float calculateCourseAverage(){
        ...
        float excludedWeight = 0.0F;
        float totalWeight = 0.0F;
        float courseAverage = 0.0F;
        // Loop to make sure all student grades are up to date
        for(int StudentID = 0; StudentID<roster.size(); StudentID++) {
            overallGrade = 0.0F;
            excludedWeight = 0.0F;
            totalWeight = 0.0F;
            for (int i = 0; i < gradebook.getCatID().size(); i++) {
        ...
        for(int StudentID = 0; StudentID<roster.size(); StudentID++) {
            courseAverage += roster.get(StudentID).getOverallGrade();
        }
        courseAverage = courseAverage/roster.size();
        gradebook.setAverageOverallGrade(courseAverage);
    ...
    public void addCategory(String categoryName, Float categoryWeight){
        ...
        if(gradebook.getCatID().isEmpty()) gradebook.getCatID().add(0);
        else gradebook.getCatID().add((gradebook.getCatID().lastElement())+1);
